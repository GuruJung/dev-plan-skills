...
Treat an existing feature as the same plan only when its saved `plan.md` equals the latest approved plan after replacing only `feature_id` with the ID in saved state, and the handoff marker remains present. Reuse that ID without creating a suffix or new metadata. If its status is `integrated`, report that it is already complete and stop.

When there is no reusable feature, read `../save-approved-plan/SKILL.md` completely and apply it as a delegated invocation from the approved handoff. If collision handling changes the ID, use the resolved saved ID for every later step. If saving fails or waits for a user choice, do not enter the run phase.

## Run

Only after a feature was newly saved or reused as the same plan, read `../run-feature/SKILL.md` completely and apply it to that ID as a delegated invocation from the approved handoff. Do not ask for an additional confirmation between saving and running.
