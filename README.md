...
최초 상태 생성에만 `scripts/record-skill-sync.sh --initialize`를 사용하며 기존 상태를 이
옵션으로 덮어쓸 수 없다. 의미 일치는 구현 검토와 독립 리뷰에서 확인한다. 설치
스크립트는 계속 `skills/`만 복사하므로 한글 원본과 개발 지침은 전역 설치에 포함되지
않는다.
